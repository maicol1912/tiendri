...
## Prompt Engineering — Patterns & Proven Examples (Steve)

### Imagen — Estructura de prompt

- Orden: subject → style → composition → lighting → camera → colors → mood → details
- Negative prompts: blurry, low quality, watermark, text errors, generic
- Aspect ratios: 16:9 (hero/banner), 1:1 (social), 9:16 (mobile/stories), 4:5 (product)

Reglas para GPT Image 2:
...
### Imagen — Ejemplo PROBADO (3D cartoon)
...
### Video — Estructura de prompt

- Orden: scene description → camera movement → duration → style → mood
...
### Video — Ejemplo PROBADO (transformación carrito → cohete)
...
Para cualquier transformación (objeto A → objeto B):
...
4. Generar Video con Kling/Runway usando ambas imágenes como keyframes

NUNCA intentar generar una "transformación" en una sola imagen. Una imagen de "mitad carrito mitad cohete" se ve mal siempre.
...
### Branding — Ember Core (única paleta activa)
...
Generar todos los assets usando el estilo dark tech de Ember Core. Para mockups de tiendas: usar las paletas de cada template (navy, rosa, verde, sage).

### Banco de prompts

Steve mantiene prompts exitosos en `ai/prompts/` (se crea cuando se genera el primer asset):
