...
const FAQ_ITEMS = [
  {
    ...
    answer: 'No. Nunca. Tu plan es fijo mensual y lo que vendas queda en tu bolsillo.',
  },
  {
          ...
          >
            ...
            >
              ...
              >
                ...
                {['Gratis', 'Pro', 'Negocio'].map((plan, i) => (
                  <div key={plan} className="px-4 py-3 text-center">
                    <span
                      className="text-xs font-semibold uppercase tracking-wider"
                      style={{
                        fontFamily: 'var(--ember-font-body)',
                        color: i === 1 ? 'var(--ember-red-400)' : 'var(--ember-text-muted)',
                      }}
                    >
                      {plan}
                    </span>
...
function PricingCarousel() {
  ...
  // Start centered on Pro (index 1) without animation
  useEffect(() => {
    const container = scrollRef.current
    if (!container) return
    ...
    const containerRect = container.getBoundingClientRect()
    const cardRect = card.getBoundingClientRect()
    const offset = card.offsetLeft - container.offsetLeft - (containerRect.width - cardRect.width) / 2
    container.scrollTo({ left: offset, behavior: 'instant' })
  }, [])

  // Listen to scroll events on the carousel container
  useEffect(() => {
    const container = scrollRef.current
    if (!container) return
