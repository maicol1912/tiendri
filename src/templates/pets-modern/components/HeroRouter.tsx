import { HERO_REGISTRY } from '@/templates/_shared/hero-variants';
import type { HeroVariant } from '@/types/templates';
import type { PromoBannerData } from '../types';
import type { StructuralVariants, TemplateRecipe } from '@/types/templates/structural-variants';

interface HeroRouterProps {
  banner: PromoBannerData;
  structuralVariants?: StructuralVariants;
  recipe?: TemplateRecipe;
  onCtaClick?: () => void;
}

const VALID_VARIANTS = new Set<HeroVariant>([
  'contained',
  'full-bleed',
  'split',
  'text-only',
  'promo-carousel',
  'gradient-promo',
  'split-carousel',
]);

export function HeroRouter({ banner, structuralVariants, recipe, onCtaClick }: HeroRouterProps) {
  const raw = structuralVariants?.heroVariant ?? recipe?.defaultHeroVariant ?? 'gradient-promo';
  const variant: HeroVariant = VALID_VARIANTS.has(raw as HeroVariant) ? (raw as HeroVariant) : 'gradient-promo';

  const HeroComponent = HERO_REGISTRY[variant];

  if (variant === 'promo-carousel' || variant === 'split-carousel') {
    return <HeroComponent mode="carousel" cards={[]} />;
  }

  if (variant === 'gradient-promo') {
    return (
      <HeroComponent
        mode="static"
        titleLight={banner.title}
        titleBold=""
        description={banner.subtitle}
        image={banner.image}
        ctaText="Ver productos"
        onCtaClick={onCtaClick}
      />
    );
  }

  return (
    <HeroComponent
      mode="static"
      titleLight={banner.title}
      titleBold=""
      description={banner.subtitle}
      image={banner.image}
      ctaText="Ver productos"
      onCtaClick={onCtaClick}
    />
  );
}
