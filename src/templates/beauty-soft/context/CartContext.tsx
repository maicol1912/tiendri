...
// Beauty Soft Template — Cart Context
// Manages cart state in localStorage.
// Key: tc_beauty_soft_cart_{slug}
// Pattern: isFirstPersistRun guard to avoid overwriting stored cart.
...
function cartKey(slug: string): string {
  return `tc_beauty_soft_cart_${slug}`;
}
...
export function CartProvider({ children, slug }: CartProviderProps) {
  ...
  // Persist to localStorage — skip first run to avoid overwrite
...
export function useCart(): CartContextValue {
  ...
  if (!ctx) {
    throw new Error("useCart must be used inside CartProvider (beauty-soft)");
  }
  return ctx;
}
