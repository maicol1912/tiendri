...
export function Header({ store }: HeaderProps) {
  const style: string = "standard";

  if (style === "minimal") {
    return (
      <header className="sticky top-0 z-40 w-full bg-[var(--t-background)]">
        <div className="max-w-7xl mx-auto flex items-center justify-center px-5 h-[60px]">
          <span
            className="font-semibold text-[var(--t-foreground)] text-lg"
    ...
    return (
      <header className="sticky top-0 z-40 w-full bg-[var(--t-background)]">
        <div className="max-w-7xl mx-auto flex flex-col items-center justify-center px-5 h-[70px]">
          ...
          {store.description && (
            <span className="text-xs text-[var(--t-muted)] truncate max-w-xs">
              {store.description}
            </span>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-5 h-[60px]">
        ...
        <div className="flex items-center gap-[9px] flex-1 min-w-0">
          <div
            ...
            style={{ backgroundColor: "var(--t-icon-pill-bg, var(--t-card))" }}
            aria-hidden="true"
          >
            <MapPin size={20} strokeWidth={1.75} className="text-[var(--t-foreground)]" />
          </div>

          <div className="flex flex-col min-w-0">
            <span
              className="truncate text-[17px] font-semibold text-[var(--t-foreground)] leading-[22px] tracking-[-0.408px]"
              style={{ fontFamily: "var(--font-heading, var(--font-sans))" }}
            >
              {store.name}
            </span>
            {store.description && (
              <span className="truncate text-sm font-normal text-[var(--t-muted)] leading-[22px] tracking-[-0.408px]">
                {store.description}
              </span>
            )}
          </div>
          ...
          >
            {store.logo ? (
              ...
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                ...
                />
                <circle cx="12" cy="7" r="4" stroke="var(--t-muted)" strokeWidth="1.75" />
              </svg>
            )}
          </div>
        </div>
