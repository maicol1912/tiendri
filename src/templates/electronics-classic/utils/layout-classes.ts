// Electronics Classic Template — Layout Class Mappings
...
export function tabStyleClasses(style: string, isActive: boolean): string {
  const map: Record<string, { active: string; inactive: string }> = {
    ...
    pills: {
      active: "text-[var(--t-category-active-text)] bg-[var(--t-primary)]/20 border border-[var(--t-primary)] rounded-[var(--t-radius-button)]",
      inactive: "text-[var(--t-text-primary)] bg-[var(--t-card-bg)] border border-[var(--t-border)] rounded-[var(--t-radius-button)] hover:border-[var(--t-primary)]/50",
    },
    bordered: {
...
export function bannerHeightClass(height: string): string {
  const map: Record<string, string> = {
    ...
    normal: "h-[280px] sm:h-[400px] md:h-[500px] lg:h-[568px]",
    tall: "h-[360px] sm:h-[480px] md:h-[600px] lg:h-[680px]",
  };
