...
// Electronics Classic Template — Cart Context
// Manages cart state in localStorage.
// Key: tc_electronics_classic_cart_{slug}
...
function cartKey(slug: string): string {
  return `tc_electronics_classic_cart_${slug}`;
}
...
export function CartProvider({ children, slug }: CartProviderProps) {
  ...
  // Persist to localStorage whenever items change (skip first run)
...
export function useCart(): CartContextValue {
  ...
  if (!ctx) {
    throw new Error("useCart must be used inside CartProvider (electronics-classic)");
  }
  return ctx;
}
